...
class EmailError {
  constructor(name, error, statusCode) {
    this.name = name
    this.error = error
    ...
    <main className="bg-primary-bg text-primary-text font-inter">
      <section className="py-16 text-center">
        ...
        <p className="text-primary-muted text-lg max-w-2xl mx-auto mb-8">
          Elevate your football journey with our global expertise. Let’s connect.
        </p>
      </section>
      ...
      <section className="pb-16">
        <div className="container mx-auto px-4 max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          {[
            ...
            <div key={title} className="bg-primary-card rounded-xl p-8 border border-divider shadow-lg">
              ...
              <p className={`text-accent-${color} font-medium text-sm`}>{text}</p>
            </div>
          ))}
        </div>
      </section>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input name="name" type="text" required className="w-full bg-primary-card border border-divider rounded-lg px-4 py-3" placeholder="Your full name" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2">Email Address</label>
                  <input name="email" type="email" required className="w-full bg-primary-card border border-divider rounded-lg px-4 py-3" placeholder="your@email.com" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium mb-2">Subject</label>
                <select name="subject" className="w-full bg-primary-card border border-divider rounded-lg px-4 py-3">
              ...
              <div>
                ...
                <textarea name="message" required rows={6} className="w-full bg-primary-card border border-divider rounded-lg px-4 py-3 resize-none" placeholder="Tell us about your goals, experience, and how we can help you..."></textarea>
              </div>
              <button type="submit" className="w-full bg-accent-red hover:bg-accent-red/90 text-white px-8 py-4 rounded-lg font-semibold">
                {status === "loading" ? (
      ...
      <section className="pb-16 md:pb-24">
        <div className="container mx-auto px-4 max-w-4xl text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { name: "Instagram", icon: "instagram", color: "red", handle: "@FootballBankOfficial" },
              { name: "YouTube", icon: "youtube", color: "red", handle: "FootballBank TV" },
              { name: "Facebook", icon: "facebook", color: "blue", handle: "FootballBank Global" },
              { name: "LinkedIn", icon: "linkedin", color: "blue", handle: "FootballBank International" },
            ].map(({ name, icon, color, handle }) => (
              <div key={name} className={`bg-primary-card rounded-xl p-6 border border-divider shadow-lg hover:border-accent-${color} transition-colors cursor-pointer`}>
